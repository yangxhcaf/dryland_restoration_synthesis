...
dist<- ggplot(evidence, aes(disturbance, fill = paradigm)) +
  geom_bar(na.rm = TRUE) +
  coord_flip() +
...
#ggsave(file="dist.svg", width=9, height=5)
...
map<-ggplot() + geom_polygon(data=world, fill="gray75", aes(x=long, y=lat, group=group))
...
ggplot(models, aes(paradigm, lrr, color = intervention)) +
  ...
  labs(x = "", y = "lrr", color = "") +
  coord_flip() +
  geom_errorbar(aes(ymin=lower, ymax=upper), size= 0.2, width=0.2, position = position_dodge(width = 0.5)) +
  geom_hline(yintercept = 0, colour="grey", linetype = "longdash")+
  theme(axis.text.x=element_text(face="bold"),
        axis.text.y=element_text(face="bold"),
        axis.title=element_text(size=12,face="bold"),
...
models <- read_csv("data/meta_outputs.csv")
ggplot(models, aes(intervention, lrr, color = paradigm)) +
  ylim(c(-2,2)) +
  geom_point(position = position_dodge(width = 0.5)) + 
  labs(x = "", y = "lrr", color = "") +
  coord_flip() +
...
p <- ggplot(models, aes(paradigm, lrr, fill = intervention)) +
  ylim(c(-2,2)) +
