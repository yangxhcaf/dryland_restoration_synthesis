...
simple <- data %>%
  ...
  summarise(n = n(), mean.lrr = mean(lrr), mean.rii = mean(rii), var.mean = mean(var.es))

simple.data <- data %>%
  group_by(study.ID, paradigm, intervention, outcome) %>%
  summarise(n = n(), mean.lrr = mean(lrr), mean.rii = mean(rii), var.mean = mean(var.es)) %>% filter(paradigm != "passive")

#viz for aggregation####
ggplot(na.omit(simple), aes(technique, n, fill = paradigm)) + 
  geom_bar(stat = "identity") + 
  coord_flip() + 
  scale_fill_brewer(palette = "Paired")

ggplot(na.omit(simple.data), aes(intervention, n)) +   geom_bar(stat = "identity", fill = "lightskyblue2") + 
  coord_flip()

ggplot(na.omit(simple.data), aes(outcome, n)) +   geom_bar(stat = "identity", fill = "lightskyblue2") + 
  coord_flip()
...
#rii####
#need to decide if using mean var.mean appropriate
#paradigm
m <- metagen(mean.rii, var.mean, studlab = study.ID, byvar = paradigm, data = simple) #fit generic meta-analysis to an object
summary(m)
...
#technique
m <- metagen(mean.rii, var.mean, studlab = study.ID, byvar = intervention, data = simple.data) #fit generic meta-analysis to an object
m
#no difference in random effects model by paradigm but fixed yes. Hetereogeneity is significantly different so a. fixed not representative and b. need a better model
#forest(m,  xlim="symmetric", plotwidth=unit(1, "cm"))
...
#measure
m <- metagen(mean.rii, var.mean, studlab = study.ID, byvar = outcome, data = simple.data) #fit generic meta-analysis to an object
m
#no difference in random effects model by paradigm but fixed yes. Hetereogeneity is significantly different so a. fixed not representative and b. need a better model
#forest(m,  xlim="symmetric", plotwidth=unit(1, "cm"))
#forest(m, sortvar = outcome)
forest(m)
radial(m)
metabias(m, method = "linreg")

#repeat with ID not study ID
