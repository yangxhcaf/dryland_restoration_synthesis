...
mdata.mean<- mdata.all %>% 
  group_by(study.ID, aridity.index, exp.length, paradigm) %>% 
  summarize(mean_lrr= mean(lrr),
  mean_var.es= mean(var.es))

mdata.mean<- mdata.mean %>%
  filter(!is.infinite(mean_var.es))

mod.1<-rma(mean_lrr,mean_var.es, mods= intervention, subset= paradigm == "active", digits=4,data=mdata.mean)

m2<-rma.mv(yi=lrr, V=var.es, mods= ~ aridity.index, random = ~ 1 | ID, data=mdata.all, sparse=TRUE)

m1 <- rma(yi=lrr, vi=var.es, data = mdata.all)
summary(m1)


m1 <- rma(lrr, var.es, mods= cbind (aridity.index, exp.length), data = mdata.all, subset = paradigm == "active")
summary(m1)
