...
mod.2<-rma.mv(yi=lrr, V=var.es, mods= ~intervention+aridity.index+exp.length, method = "REML",  test="t", random = ~ 1 | ID, data=mdata.all, sparse=TRUE)
summary(mod.2)
...
mod.5 <- rma(lrr, var.es, mods=  ~intervention+aridity.index+exp.length, data = mdata.all, subset = paradigm == "active")
summary(mod.5)
```
