...
simple.data <- data %>% group_by(study.ID) %>% summarise(mean.rii = mean(rii), error = std.error(rii))
...
m <- metagen(mean.rii, error, studlab = study.ID, data = simple.data) #fit generic meta-analysis to an object
forest(m)
