...
m1 <- metagen(LRR, LRR_var, studlab = ID, byvar = paradigm, comb.fixed=FALSE, data = metadata)
...
#interventions
m2 <- metagen(LRR, LRR_var, studlab = ID, byvar = intervention, comb.fixed=FALSE, subset = paradigm == "active", data = metadata)
summary(m2)
#metabias(m)
...
table(mdata.all$var.es)

#write.csv(mdata.all, file="mdata.all.csv")
```
...
mod.1<-rma(LRR,LRR_var, mods= ~ aridity.index, subset= paradigm == "active", digits=4,data=metadata)
mod.1
