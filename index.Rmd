...
summary(m2)
#metabias(m)
#metareg(m2, ~ aridity.index, data=mdata.all)
...
#res1 <- rma(lrr, var.es, data=mdata.all, subset=intervention=="vegetation") error
#res2 <- rma(lrr, var.es, data=mdata.all, subset=intervention=="soil") error

mod.1<-rma(lrr,var.es, mods= ~ aridity.index, subset= paradigm == "active", digits=4,data=mdata.for)
...
mod.5 <- rma(lrr, var.es, mods=  ~unit, data = mdata.all, subset = paradigm == "active")
summary(m5)
```
